...
class EyeTracker:
    ...
    def track(self, image):
        ...
        for (fX, fY, fW, fH) in faceRects:
            # grab only the face then later look for eyes in only this region!
            faceROI = image[fY:fY+fH, fX:fX+fW]
            rects.append((fX, fY, fX+fW, fY+fH))
            # look for eyes in the face image (faceROI)
            eyeRects = self.eyeCascade.detectMultiScale(faceROI, scaleFactor=1.1,
             minNeighbors=10, minSize=(20,20), flags=cv2.CASCADE_SCALE_IMAGE)
